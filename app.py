...
def three_nearest_months(data):
    ...
    if not date_columns:
        print("Không tìm thấy cột nào có định dạng ngày tháng.")
...
def calculate_view():
    ...
    if request.method == 'POST':
        ...
        if file:
            ...
            #MongoDB
            #collection_name = f"datacalculate_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            mongo.db.processed_data.delete_many({}) #xoa dbcu
            mongo.db.processed_data.insert_many(records)
        ...
        filtered_data = list(mongo.db.processed_data.find())        
        display_data = pd.DataFrame( filtered_data)
        
        three_months = three_nearest_months(display_data)
...
def download_file():
    ...
    data = pd.DataFrame(list(mongo.db.processed_data.find()))
